...
using Trophy = Maple2.Model.Game.Trophy;
using TrophyEntry = Maple2.Model.Game.TrophyEntry;
...
public partial class GameStorage {
    public partial class Request : IPlayerInfoProvider {
        ...
        public IDictionary<int, TrophyEntry> GetAccountTrophy(long accountId) {
            Model.Account? model = Context.Account.Find(accountId);
            Dictionary<int, TrophyEntry> trophies = new Dictionary<int, TrophyEntry>();
            if (model?.Trophy == null) {
                return trophies;
            }

            foreach ((int id, Model.TrophyEntry entry) in model.Trophy) {
                if (!game.trophyMetadata.TryGet(id, out TrophyMetadata? metadata)) {
                    continue;
                }
                
                trophies.Add(id, entry.Convert(metadata));
            }
            return trophies;
        }
    ...
    public bool SavePlayer(Player player, IDictionary<int, TrophyEntry> accountTrophies) {
